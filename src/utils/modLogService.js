...
const { nextCaseId, createCaseWithId, updateCase } = require('./modCases');
...
async function requireModLog(interaction) {
  if (!interaction.guild) {
    await safeReply(interaction, { ephemeral: true, content: '❌ This command can only be used in a server.' }).catch(() => {});
    return null;
  }
...
async function updateCaseLogReason(client, guildId, caseId, newReason, editorUser) {
  ...
  const { getCase } = require('./modCases');
