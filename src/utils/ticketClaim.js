const { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, allowedMentions } = require('discord.js');
const { getStaffCandidates, chooseStaff } = require('./staffV2');
const { getTicket, setTicket } = require('./ticketData');
...
async function pingAndDelete({ channel, chosen }) {
  ...
  if (pingMsg) setTimeout(() => pingMsg.delete().catch(() => {}), 4000);
}
...
async function scheduleNext({ guild, channel, settings, openerId, attempt, attemptedIds }) {
  ...
  const handle = setTimeout(async () => {
    try {
      ...
      console.error('[TICKET] claim schedule error:', e);
    }
  }, delayMs);
