...
const states = new Map(); // guildId -> state (non-serializable fields)
const { persistState, loadState, clearState } = require('../services/musicStateStore');
...
function getState(guildId) {
  ...
  // Hydrate persisted fields from Redis (non-blocking — failures are silent)
  loadState(guildId).then((saved) => {
    if (!saved) return;
    if (Array.isArray(saved.queue) && saved.queue.length > 0) st.queue = saved.queue;
    if (saved.volume != null) st.volume = saved.volume;
    if (saved.stay247 != null) st.stay247 = saved.stay247;
    if (saved.voiceChannelId) st.voiceChannelId = saved.voiceChannelId;
    if (saved.textChannelId) st.textChannelId = saved.textChannelId;
    if (saved.loopMode) st.loopMode = saved.loopMode;
  }).catch(() => {});
...
/**
 * Base args appended to EVERY yt-dlp invocation.
 *
 * YouTube bot-detection bypass options (set via .env / environment variables):
 *
 *   YTDLP_COOKIES_FILE   — path to a Netscape-format cookies.txt file.
 *                          Export from your browser with the
 *                          "Get cookies.txt LOCALLY" extension while logged
 *                          into YouTube, then set:
 *                          YTDLP_COOKIES_FILE=/data/cookies.txt
 *
 *   YTDLP_COOKIES_BROWSER — use cookies directly from a browser profile
 *                          (e.g. "chrome", "firefox", "edge").
 *                          Only works when the bot runs on a machine that
 *                          has that browser installed.
 *                          YTDLP_COOKIES_BROWSER=chrome
 *
 *   YTDLP_PO_TOKEN       — Proof-of-Origin token (optional, pairs with
 *                          YTDLP_PO_TOKEN_VISITOR_DATA).
 *                          Obtain via the yt-dlp PO-token helper:
 *                          https://github.com/yt-dlp/yt-dlp/wiki/PO-Token-Guide
 *
 *   YTDLP_PO_TOKEN_VISITOR_DATA — Visitor data string that pairs with
 *                          YTDLP_PO_TOKEN.
 *
 * If none of the above are set the bot still works for most content;
 * age-restricted or heavily bot-guarded videos will fail.
 */
function getYtDlpBaseArgs() {
  ...
  // ── Browser cookies (alternative — requires browser on same machine) ──────
    ...
    args.push('--cookies-from-browser', process.env.YTDLP_COOKIES_BROWSER.trim());
  }

  // ── PO Token (optional, reduces bot-detection for cookie-authed sessions) ─
  const poToken = process.env.YTDLP_PO_TOKEN;
  const visitorData = process.env.YTDLP_PO_TOKEN_VISITOR_DATA;
  if (poToken && visitorData) {
    args.push(
      '--extractor-args',
      `youtube:po_token=web+${poToken};visitor_data=${visitorData}`,
    );
  } else if (poToken) {
...
/**
 * searchYouTube(query) — unified YouTube search with automatic fallback.
 *
 * Tries play-dl first (faster, no subprocess). If play-dl's YouTube API
 * is broken (browseId / innertube changes), falls back to yt-dlp search
 * which is always up-to-date.
 *
 * Returns: { url, title, duration } or null if both methods fail.
 */
async function searchYouTube(query) {
  // Strategy 1: play-dl (fast, in-process)
  try {
    const results = await play.search(query, { source: { youtube: 'video' }, limit: 1 });
    const r = results?.[0];
    if (r?.url) return { url: r.url, title: r.title || query, duration: (r.durationInSec ?? 0) || null };
  } catch (e) {
    logger.debug('[MUSIC] play-dl search failed, trying yt-dlp:', e?.message ?? e);
  }

  // Strategy 2: yt-dlp search (always up-to-date)
  try {
    ...
    const raw = await new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => {
        ...
        if (parts[0]?.startsWith('http')) resolve({ url: parts[0], title: parts[1] || query, duration: parseInt(parts[2], 10) || null });
        else reject(new Error(`yt-dlp search returned no results (code ${code})`));
      });
      child.on('error', reject);
      setTimeout(() => { try { child.kill(); } catch {} reject(new Error('yt-dlp search timeout')); }, 15000);
    });
    return raw;
  } catch (e) {
    logger.debug('[MUSIC] yt-dlp search also failed:', e?.message ?? e);
  }
...
async function ensureSoundCloudToken() {
  // Use a promise so concurrent calls await the same initialization
  if (ensureSoundCloudToken._promise) return ensureSoundCloudToken._promise;
  ensureSoundCloudToken._promise = (async () => {
    try {
      const clientID = await play.getFreeClientID();
      play.setToken({ soundcloud: { client_id: clientID } });
      logger.info('[MUSIC] SoundCloud client_id set successfully');
    } catch (e) {
  ...
  return ensureSoundCloudToken._promise;
}
...
if (looksLikeUrl(query)) {
  ...
  const type = await play.validate(url);
    ...
    // YouTube video — play-dl first, yt-dlp fallback for browseId/innertube errors
    if (type === 'yt_video') {
      try {
        ...
        logger.debug('[MUSIC] play-dl video_basic_info failed (browseId/innertube issue), using yt-dlp metadata:', e?.message);
      }

      // yt-dlp fallback: fetch metadata without downloading
      try {
        const cmd = getYtDlpCommand();
        const raw = await new Promise((resolve, reject) => {
          const child = spawn(cmd, [
          ...
          child.on('error', reject);
          setTimeout(() => { try { child.kill(); } catch {} reject(new Error('timeout')); }, 15000);
        });
        const [resolvedUrl, title, dur] = raw.split('|||');
        if (resolvedUrl?.startsWith('http')) {
          return [{ title: title || 'Unknown title', url: resolvedUrl, duration: parseInt(dur, 10) || null, source: 'youtube' }];
        }
      } catch (ytErr) {
        logger.debug('[MUSIC] yt-dlp metadata fetch failed:', ytErr?.message);
      }

      // Last resort: use the URL as-is and let the stream strategies handle it
      return [{ title: 'Unknown title', url, duration: null, source: 'youtube' }];
    }

    // YouTube playlist — play-dl first, yt-dlp fallback
    if (type === 'yt_playlist') {
      try {
        ...
        logger.debug('[MUSIC] play-dl playlist_info failed, using yt-dlp:', e?.message);
      }

      // yt-dlp fallback for YouTube playlists
      const cmd = getYtDlpCommand();
      const raw = await new Promise((resolve, reject) => {
        const child = spawn(cmd, [
        ...
        child.on('close', () => resolve(out.trim()));
        child.on('error', reject);
        setTimeout(() => { try { child.kill(); } catch {} reject(new Error('yt-dlp timeout')); }, 30000);
      });

      const tracks = raw.split('\n').map((line) => {
        const [u, t, d] = line.split('|||');
        return u?.startsWith('http') ? { url: u, title: t || 'Unknown', duration: parseInt(d, 10) || null, source: 'youtube' } : null;
      }).filter(Boolean).slice(0, MAX_PLAYLIST_TRACKS);

      if (tracks.length > 0) return tracks;
      throw new Error('Could not load that YouTube playlist. It may be private or empty.');
    }

    // SoundCloud track / playlist
    // play-dl handles SoundCloud natively. If it fails (API rate limit, token issue),
    // fall back to yt-dlp which also supports SoundCloud URLs.
    if (type === 'so_track' || type === 'so_playlist') {
      try {
        ...
        logger.warn('[MUSIC] play-dl SoundCloud failed, trying yt-dlp:', scErr?.message ?? scErr);
      }

      // yt-dlp fallback for SoundCloud — handles both tracks and playlists
      try {
        const cmd = getYtDlpCommand();
        const raw = await new Promise((resolve, reject) => {
          const child = spawn(cmd, [
          ...
          child.on('close', () => resolve(out.trim()));
          child.on('error', reject);
          setTimeout(() => { try { child.kill(); } catch {} reject(new Error('yt-dlp timeout')); }, 20000);
        });

        const tracks = raw.split('\n')
          .map((line) => {
            const [url, title, dur] = line.split('|||');
            return url?.startsWith('http') ? { url, title: title || 'Unknown', duration: parseInt(dur, 10) || null, source: 'soundcloud' } : null;
          })
          .filter(Boolean)
        ...
        logger.warn('[MUSIC] yt-dlp SoundCloud fallback also failed:', ytdlpErr?.message ?? ytdlpErr);
      }

      throw new Error('Could not load that SoundCloud link. The URL may be private or SoundCloud is temporarily unavailable.');
    }
    ...
    if (type === 'sp_track' || type === 'sp_album' || type === 'sp_playlist') {
      ...
      try {
        ...
        if (spot?.type === 'track') {
          ...
          const yt = await searchYouTube(searchQ);
          if (!yt?.url) throw new Error('No matching YouTube results found for that Spotify track. Try searching by song name instead.');
          return [
            {
        ...
        if (type === 'sp_track') {
          ...
          if (title) {
            const yt = await searchYouTube(title);
            if (yt?.url) {
              return [
                {
        ...
        // For sp_track: oEmbed fallback was already attempted above and failed.
        // For sp_album/sp_playlist: play-dl requires Spotify OAuth tokens.
        // Instead of a confusing technical error, give the user clear options.
        const isSingle = type === 'sp_track';
        if (isSingle) {
          throw new Error('Could not find a YouTube match for that Spotify track. Try searching by song name instead.');
        }
        // For playlists/albums: give actionable guidance without requiring play-dl auth flow
        const kind = type === 'sp_album' ? 'album' : 'playlist';
        throw new Error(
          `Spotify ${kind}s cannot be queued without Spotify credentials configured on the bot.\n` +
          `**Options:**\n` +
          `• Copy the ${kind} title and search it on YouTube instead\n` +
          `• Ask your server admin to set up Spotify credentials (see .env.example SPOTIFY_* vars)\n` +
          `• Paste individual Spotify track links instead of the full ${kind}`
        );
      }
    }
  ...
  // Search flow: unified searchYouTube() tries play-dl then yt-dlp automatically
  let searchUrl = null;
  ...
  if (searchResult?.url) {
    ...
    searchTitle = searchResult.title || null;
    searchDuration = searchResult.duration || null;
  }

  // Legacy yt-dlp search block below is now handled inside searchYouTube()
  // Keeping this structure for the result consumption code below.
  if (!searchUrl) {
    try {
      const cmd = getYtDlpCommand();
...
async function playTrackInternal(st, track) {
  ...
  // ── Strategy 3: play-dl ────────────────────────────────────────────────────
  // play-dl supports YouTube and SoundCloud only — skip other sources to avoid
  // "Invalid URL" errors that pollute the error list and mislead diagnostics.
  const playdlSupported = isYouTubeUrl(track.url) || /soundcloud\.com/i.test(track.url);
  if (st.player.state.status !== AudioPlayerStatus.Playing && playdlSupported) {
    try {
      // play-dl: guard against the `Cannot read properties of undefined (reading 'expiry')`
      // crash that occurs in play-dl 1.9.x when no YouTube OAuth tokens are configured.
      // is_expired() internally reads token.expiry — wrap it so the entire strategy
      // doesn't blow up just because tokens are absent.
      try {
        if (play.is_expired && play.is_expired()) {
          await play.refreshToken().catch(() => {});
        }
      } catch {
        // No tokens configured — that's fine, play.stream() can still work
        // for non-age-restricted content via the innertube API.
      }
  ...
  if (st.player.state.status !== AudioPlayerStatus.Playing) {
    // Build a user-friendly error with actionable fix steps
    const ytdlpOutdated = errors.some(e => e.includes('0.0%') || e.includes('0.1%') || e.includes('403'));
    const opusMissing = errors.some(e => e.includes('opus') || e.includes('opusscript') || e.includes('prebuild'));
    const tips = ['Make sure yt-dlp is up to date: replace bin/yt-dlp.exe with the latest from https://github.com/yt-dlp/yt-dlp/releases'];
    if (opusMissing) tips.push('Opus encoder missing: run `npm install opusscript@0.1.1` then restart the bot');
    if (!process.env.YTDLP_COOKIES_FILE) tips.push('Set YTDLP_COOKIES_FILE in .env to bypass YouTube bot detection (see .env.example)');
    throw new Error(
...
async function leave(guildId, reason = 'Leave') {
  ...
  // Clear persisted state from Redis when leaving cleanly
  try {
...
async function restore247ForClient(client) {
  ...
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      logger.error({ err: e, guildId: guild?.id }, '[MUSIC] 24/7 restore error: ' + (e?.message || String(e)));
    }
  }
}
