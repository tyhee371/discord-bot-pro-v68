const { db } = require('../db');
// Lazy import to avoid circular — modStats imports modCases
let _indexCase = null;
function getIndexCase() {
  if (!_indexCase) _indexCase = require('./modStats').indexCase;
  return _indexCase;
}
...
async function createCase(guildId, data) {
  ...
  // Index in analytics store (non-blocking)
  getIndexCase()(guildId, caseId).catch(() => {});
